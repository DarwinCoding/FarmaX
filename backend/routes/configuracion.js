...
router.put("/stock-minimo-global", (req, res) => {
  try {
...
router.put("/dias-alerta-caducidad", (req, res) => {
  try {
...
router.get("/backup", (req, res) => {
  try {
    if (!req.session.usuario || req.session.usuario.rol !== "admin") {
      return res.status(403).json({ error: "Solo los administradores pueden descargar respaldos." });
    }
